...
class CustomDataset(Dataset):
    def __init__(self, tensor1, tensor2):
        """
        tensor1: 形状为 [N, channel, height, width]
        tensor2: 形状为 [N, channel]
        """
        self.tensor1 = tensor1
        self.tensor2 = tensor2
        assert len(tensor1) == len(tensor2), "两个张量的第一个维度必须相等"
...
class Trainer(object):
    def __init__(
            ...
            rho,
            BC,
            device=torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu"),  # 提供默认值
            train_batch_size=16,
            train_lr=1e-4,
        ...
        dl = DataLoader(CustomDataset(rho, BC), batch_size=train_batch_size, shuffle=True, pin_memory=True)
            ...
            while self.step < self.train_num_steps:
                with accelerator.accumulate(self.model):
                    ...
                    if accelerator.is_main_process:
                        ...
                        l_bc_x0_val = loss_components.get('L_BC_x0', torch.tensor(0.0)).item()
