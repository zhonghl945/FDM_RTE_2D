...
class Unet3D(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.in_channels = solution_channels + conditioning_channels
        ...
        self.final_conv_T_eps = nn.Conv3d(init_dim, solution_channels, 1)
        self.head_S_x0 = nn.Sequential(
            nn.AdaptiveAvgPool3d((1, None, None)),
            nn.Conv2d(init_dim, init_dim // 2, 3, padding=1),
            nn.SiLU(),
            nn.Conv2d(init_dim // 2, conditioning_channels, 1)
        )
    ...
    def forward(self, x, time):
        ...
        T_eps_pred = self.final_conv_T_eps(x_final_feat)
        S_x0_pred_features = self.head_S_x0[0](x_final_feat).squeeze(2)
        S_x0_pred = self.head_S_x0[1:](S_x0_pred_features)

        return T_eps_pred, S_x0_pred
...
class GaussianDiffusion3D(nn.Module):
    def __init__(
            ...
            model: Unet3D,
            *,
            T_shape: tuple,  # Shape: (Nt, H, W), e.g., (64, Nx, Nx)
            timesteps=1000,
            beta_schedule='cosine',
            lambda_S_x0=1.0,  # Loss weight for the direct S (x0) prediction
            freeze_indices_T: tuple = (slice(None), slice(None), slice(None)),
            rescaler=1.0,
    ):
        super().__init__()
        self.model = model
        ...
        self.nt = T_shape[0]  # Temporal dimension for T
        self.H = T_shape[1]
        self.W = T_shape[2]

        # Assuming Unet3D input_S_channels=1 for this strategy
        self.num_input_S_channels = 1  # Fixed for this strategy
        self.diffusion_input_channels = 1 + self.num_input_S_channels  # 1 for T, 1 for expanded S

        self.data_shape_diffusion = (self.diffusion_input_channels, self.nt, self.H, self.W)

        self.num_timesteps = int(timesteps)
        self.lambda_S_x0 = lambda_S_x0

        self.freeze_indices_train = (slice(None), 0, *freeze_indices_T)  # For T channel (index 0)
        self.freeze_indices_sample = (slice(None), 0, *freeze_indices_T)
    ...
    def p_losses(self, T_start, S_start, t):
        # T_start: [B, Nt, H, W]
        # S_start: [B, H, W]
        # t: [B]
        B = T_start.shape[0]

        T_start_expanded_ch = T_start.unsqueeze(1)  # [B, 1, Nt, H, W]

        # S_start: [B, H, W] -> [B, 1, H, W] -> [B, 1, 1, H, W] -> [B, 1, Nt, H, W]
        S_start_expanded_temporal = S_start.unsqueeze(1).unsqueeze(1).expand(
            B, self.num_input_S_channels, self.nt, self.H, self.W
        )

        x_start_combined = torch.cat((T_start_expanded_ch, S_start_expanded_temporal), dim=1)
        ...
        # 3. Apply Freezing Constraint to T part of x_t and noise_target
        x_t_frozen = x_t.clone()
        x_t_frozen[self.freeze_indices_train] = T_start_expanded_ch[self.freeze_indices_train]

        noise_target_frozen = noise_target.clone()
        noise_target_frozen[self.freeze_indices_train] = 0.  # More precise for T channel only

        # 4. Predict using the model
        T_eps_pred, S_x0_pred = self.model(x_t_frozen, t)
        # T_eps_pred: [B, 1, Nt, H, W]
        # S_x0_pred: [B, 1, H, W]
        ...
        # 扩散损失
        loss_T_eps = F.mse_loss(T_eps_pred, noise_target_frozen[:, 0:1, ...])
        loss_S_direct_x0 = F.mse_loss(S_x0_pred.squeeze(1), S_start)
        L_diffusion = loss_T_eps + self.lambda_S_x0 * loss_S_direct_x0

        total_loss = L_diffusion
        # 日志记录
        loss_dict['total_loss'] = total_loss
        loss_dict['S_x0_loss'] = loss_S_direct_x0.detach()

        return loss_dict

    @torch.no_grad()
    def p_sample_standard(self, x_k, k, x_f_known_normalized_T):
        # x_k: [B, 2, Nt, H, W]
        # x_f_known_normalized_T: [B, Nt, H, W] (normalized known T for freezing)
        B = x_k.shape[0]
        batched_k = torch.full((B,), k, device=self.device, dtype=torch.long)

        x_k_frozen = x_k.clone()
        x_f_known_norm_T_chan = x_f_known_normalized_T.unsqueeze(1)  # [B, 1, Nt, H, W]
        x_k_frozen[self.freeze_indices_sample] = x_f_known_norm_T_chan[self.freeze_indices_sample]

        T_eps_pred, S_x0_pred = self.model(x_k_frozen, batched_k)
        # T_eps_pred: [B, 1, Nt, H, W]
        # S_x0_pred: [B, 1, H, W]

        # Estimate clean T from its noise prediction
        # x_k_frozen[:, 0:1, ...] is the T part of the noisy input
        T_0_pred = self.predict_x0_from_eps(x_k_frozen[:, 0:1, ...], batched_k, T_eps_pred)

        # Temporal expand predicted clean S_x0_pred
        S_x0_pred_expanded_temporal = S_x0_pred.unsqueeze(1).expand(
            B, self.num_input_S_channels, self.nt, self.H, self.W
        )

        x_0_pred_combined = torch.cat((T_0_pred, S_x0_pred_expanded_temporal), dim=1)
        x_0_pred_combined[self.freeze_indices_sample] = x_f_known_norm_T_chan[self.freeze_indices_sample]
    ...
    @torch.no_grad()
    def p_sample_loop(self, batch_size, x_f_known_T_orig, rescaler):
        # x_f_known_T_orig: [B, Nt, H, W] (original scale)
        img = torch.randn((batch_size, *self.data_shape_diffusion), device=self.device)

        x_f_known_T_orig = x_f_known_T_orig.to(self.device)
        rescaler = rescaler.to(self.device)
        x_f_known_normalized_T = 2 * (x_f_known_T_orig / rescaler) - 1  # Assuming standard normalization

        # tqdm can be added in the calling script (e.g., Trainer or inference script)
        for k_val in reversed(range(0, self.num_timesteps)):
            img = self.p_sample_standard(img, k_val, x_f_known_normalized_T)
        return img  # This is the predicted x0_combined (normalized) [B, 2, Nt, H, W]

    @torch.no_grad()
    def sample(self, batch_size: int, x_f_known_T_orig, rescaler):
        # x_f_known_T_orig: [B, Nt, H, W] (original scale, for freezing)

        x0_approx_combined_norm = self.p_sample_loop(batch_size, x_f_known_T_orig, rescaler)
        # x0_approx_combined_norm is [B, 2, Nt, H, W]

        # Better: Use a final model call with t=0 for clean predictions, especially for S.
        ...
        final_T_eps_pred, final_S_x0_pred_norm = self.model(x0_approx_combined_norm, k_zero)

        # For T, use the prediction from x0_approx_combined_norm directly or re-derive
        # If using x0_approx_combined_norm directly for T:
        T_0_final_norm = x0_approx_combined_norm[:, 0:1, ...].squeeze(1).clamp(-1., 1.)

        # For S, use the direct x0 prediction from the model at t=0
        S_0_final_norm = final_S_x0_pred_norm.squeeze(1).clamp(-1., 1.)  # [B, H, W]

        return T_0_final_norm, S_0_final_norm  # Return normalized predictions

    def forward(self, data_tuple):  # For training
        T_start, S_start = data_tuple  # T_start: [B,Nt,H,W], S_start: [B,Nx,Ny]

        T_start = T_start.to(self.device)
        S_start = S_start.to(self.device)

        if T_start.shape[2] != S_start.shape[2] and T_start.shape[2] != S_start.shape[2]:
            raise ValueError(f"Spatial dimension of T_start ({T_start.shape[1]}) "
                             f"must match spatial dimension of S_start ({S_start.shape[1]}) for this strategy.")

        b = T_start.shape[0]
        t = torch.randint(0, self.num_timesteps, (b,), device=self.device).long()

        return self.p_losses(T_start, S_start, t)
